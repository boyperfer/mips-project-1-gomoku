...
parse_string:           
	...
	multiDigits:         
		...
		add $t6, $t6, $t1			# sumtotal=sumtotal+t1
	...
	mul $t5, $t6, $s2				# $t5 (array pointer) <-- row index* row size 
	add $t5, $t5, $t7               # $t5 <-- row index* row size + column index
	add $t5, $s0, $t5				# $t5 <-- base address + (row index * row size + column index)
